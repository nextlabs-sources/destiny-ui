policyStudio.controller('ResourceListController', ['$scope', '$http', '$state', 'resourceService', 'loggerService', '$window', '$stateParams', '$uibModal', 'autoCloseOptionService', 'dialogService', '$filter', 'viewCacheService', 'userService', 'configService', '$q', function($scope,
  $http, $state,
  ...
  var resetSearchCriteria = function() {
    ...
    $scope.typeChanged();
    // $scope.updateType();
    $scope.searchCriteria.sortBy = $scope.getResourceSortBy();
  };

  $scope.setResourceSortBy = function(sort) {
    $scope.searchCriteria.sortBy = sort;
    localStorage.setItem('resource.sortBy', JSON.stringify(sort));
  };

  $scope.getResourceSortBy = function() {
    return localStorage.getItem('resource.sortBy') === null ? $scope.searchOptions.sortOptions[0] : JSON.parse(localStorage.getItem('resource.sortBy'));
  }
  ...
  $scope.updateTag = function() {
    ...
    if ($scope.searchOptions.$allTagChecked) {
      ...
      $scope.searchOptions.tagLabel = $filter('translate')('resourcesearch.title.NoTags');
      $scope.searchCriteria.tags.push($filter('translate')("resourcesearch.title.NoTags"));
    } else {
      $scope.searchOptions.tagLabel = '';
